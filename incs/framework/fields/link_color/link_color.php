 ...
 *
 */
if ( ! class_exists( 'WUPPO_Field_link_color' ) ) {
  class WUPPO_Field_link_color extends WUPPO_Fields {
    ...
    public function render() {
      ...
      $color_props = array(
        'color'    => esc_html__( 'Normal', 'wuppo' ),
        'hover'    => esc_html__( 'Hover', 'wuppo' ),
        'active'   => esc_html__( 'Active', 'wuppo' ),
        'visited'  => esc_html__( 'Visited', 'wuppo' ),
        'focus'    => esc_html__( 'Focus', 'wuppo' )
      );
      ...
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<div class="wuppo--left wuppo-field-color">';
          echo '<div class="wuppo--title">'. esc_attr( $color_prop_value ) .'</div>';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="wuppo-color"'. $default_attr . $this->field_attributes() .'/>';
          echo '</div>';
