 ...
 *
 */
if ( ! class_exists( 'WUPPO_Profile_Options' ) ) {
  class WUPPO_Profile_Options extends WUPPO_Abstract{
    ...
    public function render_profile_form_fields( $profileuser ) {
      ...
      $errors     = ( ! empty( $profile_id ) ) ? get_user_meta( $profile_id, '_wuppo_errors_'. $this->unique, true ) : array();
      $errors     = ( ! empty( $errors ) ) ? $errors : array();
      $class      = ( $this->args['class'] ) ? ''. $this->args['class'] : '';

      if ( ! empty( $errors ) ) {
        delete_user_meta( $profile_id, '_wuppo_errors_'. $this->unique );
      }

      echo '<div class="wuppo wuppo-profile-options wuppo-onload'. esc_attr( $class ) .'">';

      wp_nonce_field( 'wuppo_profile_nonce', 'wuppo_profile_nonce'. $this->unique );
      ...
      foreach ( $this->sections as $section ) {
        ...
        if ( ! empty( $section['fields'] ) ) {
          ...
          foreach ( $section['fields'] as $field ) {
            ...
            WUPPO::field( $field, $this->get_meta_value( $profile_id, $field ), $this->unique, 'profile' );
    ...
    public function save_profile( $user_id ) {
      ...
      $data     = array();
      $errors   = array();
      $noncekey = 'wuppo_profile_nonce'. $this->unique;
      $nonce    = ( ! empty( $_POST[ $noncekey ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ $noncekey ] ) ) : '';

      if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || ! wp_verify_nonce( $nonce, 'wuppo_profile_nonce' ) ) {
        return $user_id;
      }
      ...
      $data = apply_filters( "wuppo_{$this->unique}_save", $data, $user_id, $this );

      do_action( "wuppo_{$this->unique}_save_before", $data, $user_id, $this );
      ...
      if ( empty( $data ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_user_meta( $user_id, '_wuppo_errors_'. $this->unique, $errors );
        }

      }

      do_action( "wuppo_{$this->unique}_saved", $data, $user_id, $this );

      do_action( "wuppo_{$this->unique}_save_after", $data, $user_id, $this );
