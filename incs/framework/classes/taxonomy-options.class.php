 ...
 *
 */
if ( ! class_exists( 'WUPPO_Taxonomy_Options' ) ) {
  class WUPPO_Taxonomy_Options extends WUPPO_Abstract{
    ...
    public function render_taxonomy_form_fields( $term ) {
      ...
      $errors    = ( ! empty( $term_id ) ) ? get_term_meta( $term_id, '_wuppo_errors_'. $this->unique, true ) : array();
      $errors    = ( ! empty( $errors ) ) ? $errors : array();
      $class     = ( $this->args['class'] ) ? ' '. $this->args['class'] : '';

      if ( ! empty( $errors ) ) {
        delete_term_meta( $term_id, '_wuppo_errors_'. $this->unique );
      }

      wp_nonce_field( 'wuppo_taxonomy_nonce', 'wuppo_taxonomy_nonce'. $this->unique );

      echo '<div class="wuppo wuppo-taxonomy wuppo-show-all wuppo-onload wuppo-taxonomy-'. esc_attr( $classname ) .'-fields '. esc_attr( $class ) .'">';
        ...
        if ( $taxonomy === $this->taxonomy ) {
          ...
          if ( ! empty( $section['fields'] ) ) {
            foreach ( $section['fields'] as $field ) {
              ...
              WUPPO::field( $field, $this->get_meta_value( $field, $term_id ), $this->unique, 'taxonomy' );
    ...
    public function save_taxonomy( $term_id ) {
      ...
      $data     = array();
      $errors   = array();
      $noncekey = 'wuppo_taxonomy_nonce'. $this->unique;
      $nonce    = ( ! empty( $_POST[ $noncekey ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ $noncekey ] ) ) : '';
      $taxonomy = ( ! empty( $_POST[ 'taxonomy' ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ 'taxonomy' ] ) ) : '';

      if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || ! wp_verify_nonce( $nonce, 'wuppo_taxonomy_nonce' ) ) {
        return $term_id;
      }
      ...
      $data = apply_filters( "wuppo_{$this->unique}_save", $data, $term_id, $this );

      do_action( "wuppo_{$this->unique}_save_before", $data, $term_id, $this );
      ...
      if ( empty( $data ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_term_meta( $term_id, '_wuppo_errors_'. $this->unique, $errors );
        }

      }

      do_action( "wuppo_{$this->unique}_saved", $data, $term_id, $this );

      do_action( "wuppo_{$this->unique}_save_after", $data, $term_id, $this );
